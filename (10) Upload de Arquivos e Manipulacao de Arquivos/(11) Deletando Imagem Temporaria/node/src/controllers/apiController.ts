...
export const uploadFile = async(req: Request, res: Response) => {
   ...
   if(req.file) {
      ...
      await sharp(req.file.path) // Pega a imagem
         .resize(300, 300, {
            fit: sharp.fit.cover,
               // .fill = Preenche comprimindo a imagem
               // .cover = Preenche mantendo a proporção, pegando do meio da imagem
            position: 'bottom' // Posição do foco da imagem
         }) // Gera um campo com esse tamanho e redimenciona a imagem
            //.resize(width, height)
         .toFormat('jpeg')
         .toFile(`./public/media/${filename}`) // Diretorio para salvamento

      await unlink(req.file.path) // Delete o arquivo temporario
